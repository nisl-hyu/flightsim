...
k_opto = 7e-1*ones(1,length(ep_vec)); k_opto(ep_vec>4) = 0.5e-1;
...
wo = 0.55*rand(outputs,units);
alpha = 0.00015; count = 0;
...
legend('MSE auto-tuned'); ylabel('MSE (degree)');
...
dydt=[y(2);
      (-beta*y(2)+y(3)+y(4))/I;
      (-y(3)+P(theta_bar_now-y(1))+(dot_theta_bar_now-y(2))*V(theta_bar_now-y(1)))/tau_bar;
      (-y(4)+(dot_theta_grating_now-y(2))*Vgrat+y(5))/tau_opto;
      (-beta*y(5)+y(3)*ro)/I];
end
