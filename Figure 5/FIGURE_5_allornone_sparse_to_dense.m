% FIGURE 5
% All-or-none sparse to dense background
...
for ep=1:length(ep_vec)
    [t01, y01] = ode45(@(t,y) sim_effc(t,y,theta_bar,dot_theta_bar,...
        dot_theta_grating,y0(:,1),t0,k_opto(ep)),t0, [0 0 0 0]');
...
legend('Vgrtng per ep'); ylabel('Vgrtng (Nm)')
...
function dydt=sim_effc(t,y, theta_bar, dot_theta_bar,dot_theta_grating,y0,t0,Vgrat)
global wh wo ro_global
theta_bar_now=interp1(t0,theta_bar,t);
dot_theta_bar_now=interp1(t0,dot_theta_bar,t);
...
P=@(theta)+10e-1*sin(theta); % pos to torque e-11
V=@(theta)1e-2*(20-2*theta^2); % vel to torque e-12
if y(3)>0.015e-10 || y(3)<-0.015e-10
    k_opto = 0;
    % supp = 1e-10;
elseif y(3)<=0.015e-10 || y(3)>=-0.015e-10
    k_opto = Vgrat;
    % supp = 0;
end
