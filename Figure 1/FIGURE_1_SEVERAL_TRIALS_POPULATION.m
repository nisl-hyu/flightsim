% FIGURE 1
% Several trials for a single fly and a population of flies
...
% Clockwise wba
figure(fg);
haxes3=subplot(2,3,hz(k)); hold on; ylim([-150 100])
plot(trial_idx(1:downs:end)*dt,V2deg*squeeze(wba_tr(flyn,find(wba_tr(flyn,:,1)~=0),1:downs:end)),'black','Linewidth',1)
...
haxes4=subplot(2,3,vt(k)); hold on; ylim([-65 65])
plot(trial_idx(1:downs:end)*dt,V2deg*wba_m(:,1:downs:end),'black','Linewidth',1)
